...
class EventEmittingModule(TonikaModule):
    ...
    async def _destroy(self):
        """Emit event during destroy"""
        self.emit("module:destroyed", {"module": self.name})
...
class TestTonikaModuleLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_module_cannot_initialize_twice(self, fresh_bus):
        """Test that module cannot be initialized twice"""
        module = SimpleModule()
        await module.init()
    ...
    @pytest.mark.asyncio
    async def test_module_cannot_destroy_before_init(self, fresh_bus):
        """Test that module cannot be destroyed before initialization"""
        module = SimpleModule()

        # Try to destroy without init
...
class TestTonikaModuleProperties:
    ...
    def test_module_bus_reference(self, fresh_bus):
        """Test that module has reference to bus"""
        module = SimpleModule()

        assert module.bus is fresh_bus
        assert module.bus is TonikaBus()

    def test_module_subscriptions_tracking(self):
        """Test that module tracks its subscriptions"""
        module = SimpleModule()

        assert hasattr(module, '_subscriptions')
        assert isinstance(module._subscriptions, list)
