"""
Comprehensive test suite for TonikaBus
...
# Import from your actual location
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent.parent / "src"))

from tonika_bus.core.bus import TonikaBus, EventHandler
...
class TestTonikaBusAsync:
    ...
    @pytest.mark.asyncio
    async def test_wait_for_resolves_immediately_if_already_emitted(self, fresh_bus):
        """Test that wait_for can resolve immediately"""
        # Emit first
        fresh_bus.emit("test:event", {"data": 999})
        
        # This should resolve immediately (within the same event loop iteration)
        # Note: Due to how wait_for works, it waits for FUTURE events
        # So we need to emit AFTER starting to wait
        
        async def emit_after_wait():
            await asyncio.sleep(0.01)
            fresh_bus.emit("test:event", {"data": 999})
        
        asyncio.create_task(emit_after_wait())
        event = await fresh_bus.wait_for("test:event", timeout_ms=500)
        
        assert event.detail == {"data": 999}
...
class TestTonikaBusEdgeCases:
    ...
    def test_handler_modifying_event_detail(self, fresh_bus):
        """Test that handlers can't affect other handlers via shared detail"""
        results = []
        
        def handler1(event):
            if isinstance(event.detail, dict):
                event.detail["modified"] = True
            results.append(("handler1", event.detail.copy() if isinstance(event.detail, dict) else event.detail))
        
        def handler2(event):
            results.append(("handler2", event.detail.copy() if isinstance(event.detail, dict) else event.detail))
        
        fresh_bus.on("test:event", handler1)
        fresh_bus.on("test:event", handler2)
        
        fresh_bus.emit("test:event", {"original": True})
        
        # Both handlers see the same object (Python passes by reference)
        # This is expected behavior - handlers should not modify event details
    ...
    def test_concurrent_subscriptions(self, fresh_bus):
        """Test adding handlers while emitting events"""
        call_counts = {"h1": 0, "h2": 0, "h3": 0}
        
        def handler1(event):
            call_counts["h1"] += 1
            # Add another handler during execution
            fresh_bus.on("test:event", handler3)
        ...
        # handler3 should only be called on second emit
        assert call_counts["h1"] == 2
        assert call_counts["h2"] == 2
